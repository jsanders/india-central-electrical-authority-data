...
do
  ...
  do
    wget --no-clobber http://cea.nic.in/archives/coal_monthly/${month}${year}.pdf -O coal/${month}${year}.pdf >> out.log 2>&1
    wget --no-clobber http://www.cea.nic.in/reports/monthly/fuel_sup_consm_rep/gas%20based/${month}${year}.pdf -O gas/${month}${year}.pdf >> out.log 2>&1
    wget --no-clobber http://cea.nic.in/archives/hydro_reserve/${month}${year}.pdf -O hydro/${month}${year}.pdf >> out.log 2>&1
  done
done
